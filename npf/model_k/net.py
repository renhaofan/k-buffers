...
class KFN(nn.Module):
    def __init__(self, args, chan_embed=48) -> None:
        ...
        self.in_dim = args.dim * args.points_per_pixel
        # fuse feature to keep same "in_dim" of UNet compared with baseline
        self.apt_conv = nn.Conv2d(in_channels=self.in_dim, out_channels=args.dim, kernel_size=1)

        self.act = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.conv1 = nn.Conv2d(self.in_dim, chan_embed, 3, padding=1)
        self.conv2 = nn.Conv2d(chan_embed, chan_embed, 3, padding=1)
        self.conv3 = nn.Conv2d(chan_embed, self.in_dim, 1)
