...
from model_k.renderer_k import *
...
if __name__ == '__main__':
    ...
    if args.version == 1:
        renderer = Renderer_k_v1(args)
    elif args.version == 2:
        renderer = Renderer_k_v2(args)
    elif args.version == 3:
        renderer = Renderer_k_v3(args)
    elif args.version == 4:
        renderer = Renderer_k_v4(args)
    else:
        sys.exit(1)
    ...
    # Optimizer
    opt_para = []
    opt_para.append({"params": renderer.mlp_rect.parameters(), "lr": 1e-4})
    opt_para.append({"params": renderer.kfn.parameters(), "lr": 1.5e-4})
    ...
    if args.xyznear:
        ...
        train_buf_id, test_buf_id = load_idx(args)  # cpu 100 800 800 k
        xyz_o = None
    else:
    ...
    if args.ckpt is not None:
        ...
        renderer.mlp_rect.load_state_dict(torch.load(join(args.ckpt,'mlp_rect.pth')))
        renderer.mlp.load_state_dict(torch.load(join(args.ckpt,'mlp.pth')))
        renderer.kfn.load_state_dict(torch.load(join(args.ckpt,'kfn.pth')))
        renderer.unet.load_state_dict(torch.load(join(args.ckpt,'unet.pth')))
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            idbuf = train_buf_id[idx].to(args.device)
            ...
            # BUG: sometimes illage memory access
            #del output
            #torch.cuda.empty_cache()
        ...
        if epoch % args.test_freq == 0:
            ...
            with torch.autograd.no_grad():
                ...
                for i, batch in enumerate(test_loader):
                    ...
                    idbuf = test_buf_id[idx].to(args.device)
            ...
            if test_psnr > best_psnr:
                ...
                torch.save(renderer.mlp_rect.state_dict(), join(back_path,'mlp_rect.pth'))
                torch.save(renderer.mlp.state_dict(), join(back_path,'mlp.pth'))
                torch.save(renderer.kfn.state_dict(), join(back_path,'kfn.pth'))
                torch.save(renderer.unet.state_dict(), join(back_path,'unet.pth'))
